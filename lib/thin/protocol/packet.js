...
class ReadPacket extends BaseBuffer {
  ...
  _readBytesWithLength(numBytes) {
    ...
    while (true) {
      const numBytesInChunk = this.caps.supportsBigChunkClr ?
        this.readUB4() : this.readUInt8();
  ...
  skipBytesChunked() {
    ...
    if (numBytes !== constants.TNS_LONG_LENGTH_INDICATOR) {
      ...
      while (true) {
        const tempNumBytes = this.caps.supportsBigChunkClr ?
          this.readUB4() : this.readUInt8();
        if (tempNumBytes === 0)
          break;
...
class WritePacket extends BaseBuffer {
  ...
  writeBytesWithLength(value) {
    if (value.length <= constants.TNS_MAX_SHORT_LENGTH ||
        this.caps.supportsBigChunkClr) {
      super.writeBytesWithLength(value);
      return;
    }
    ...
    while (offset < value.length) {
      const chunkLength = Math.min(constants.TNS_OLD_CLR_CHUNK_SIZE,
        value.length - offset);
      this.writeUInt8(chunkLength);
      this.writeBytes(value.subarray(offset, offset + chunkLength));
      offset += chunkLength;
    }
    this.writeUInt8(0);
  }
