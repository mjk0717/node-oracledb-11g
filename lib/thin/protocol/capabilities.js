...
class Capabilities {
  ...
  adjustForServerCompileCaps(serverCaps, nscon) {
    ...
    const ttc3Index = constants.TNS_CCAP_TTC3;
    this.supportsBigChunkClr = ttc3Index < serverCaps.length &&
      (serverCaps[ttc3Index] & constants.TNS_CCAP_BIG_CHUNK_CLR) !== 0;
    if (!this.supportsBigChunkClr) {
      this.compileCaps[ttc3Index] &= ~constants.TNS_CCAP_BIG_CHUNK_CLR;
    }
