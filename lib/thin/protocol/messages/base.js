...
class Message {
  ...
  processErrorInfo(buf) {
    ...
    const rowCount = buf.readUB4();             // current row number
    const errorNum = buf.readUB2();             // error number
    ...
    if (buf.caps.ttcFieldVersion < constants.TNS_CCAP_FIELD_VERSION_12_1) {
      // Before Oracle Database 12.1, three additional OER data fields follow
      // the logical rowid and the extended error and row count fields are not
      // present.
      for (let i = 0; i < 3; i++) {
      ...
      this.errorInfo.num = errorNum;
      this.errorInfo.rowCount = rowCount;
    } else {
      // batch error codes
