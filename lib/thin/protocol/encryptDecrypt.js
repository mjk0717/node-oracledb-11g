...
const legacy10GAuth = require('./legacy10GAuth.js');
...
class EncryptDecrypt {
  ...
  /**
   * Updates authObject for the legacy Oracle 10G verifier protocol.
   */
  updateVerifierData10G(sessionData, username, password, newPassword, authObj) {
    const keys = legacy10GAuth.deriveSessionKeys(
      sessionData, username, password
    );
    authObj.sessionKey = keys.sessionKey;
    authObj.comboKey = keys.comboKey;

    const passwordBytes = Buffer.from(password, 'utf8');
    const newPasswordBytes = newPassword ?
      Buffer.from(newPassword, 'utf8') : undefined;
    this._setEncryptedPasswordBuffers(
      passwordBytes, newPasswordBytes, authObj.comboKey, authObj
    );
  }
