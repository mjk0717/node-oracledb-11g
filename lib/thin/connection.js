...
class ThinConnectionImpl extends ConnectionImpl {
  ...
  async _getExecuteMessage(statement, numIters, binds, options,
    ...
    if (this._protocol.caps.ttcFieldVersion <
        constants.TNS_CCAP_FIELD_VERSION_12_1) {
      if (options.batchErrors) {
        errors.throwNotImplemented(
          'batch errors with Oracle Database 11g Release 2'
        );
      }
      if (options.dmlRowCounts) {
        errors.throwNotImplemented(
          'DML row counts with Oracle Database 11g Release 2'
        );
      }
    }
  ...
  async connect(params) {
    ...
    try {
      ...
      if (this._protocol.caps.protocolVersion === constants.TNS_VERSION_11_2) {
        if (this.nscon.ntAdapter.secure) {
          errors.throwNotImplemented(
            'TCPS connections to Oracle Database 11g Release 2'
          );
        }
        if (params.externalAuth) {
          errors.throwNotImplemented(
            'external authentication with Oracle Database 11g Release 2'
          );
        }
      }
